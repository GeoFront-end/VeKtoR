...
window.addEventListener("DOMContentLoaded", () => {
    ...
    function toggleMenu() {
        ...
        if (!isMenuOpen) {
            ...
            gsap.to(linkHighlighter, { y: "0%", duration: 1, delay: 1, ease: "power2.out" });
    ...
    menuLinksContainers.forEach((link) => {
        link.addEventListener("mouseenter", () => {
            ...
            gsap.to(linkCopy[0].querySelectorAll(".char"), { y: "-110%", duration: 0.75, ease: "expo.inOut", stagger: 0.05 });
            gsap.to(linkCopy[1].querySelectorAll(".char"), { y: "0%", duration: 0.75, ease: "expo.inOut", stagger: 0.05 });
        });

        link.addEventListener("mouseleave", () => {
            if (window.innerWidth < 979) return;
            const linkCopy = link.querySelectorAll("a span");
            gsap.to(linkCopy[1].querySelectorAll(".char"), { y: "110%", duration: 0.75, ease: "expo.inOut", stagger: 0.05 });
            gsap.to(linkCopy[0].querySelectorAll(".char"), { y: "0%", duration: 0.75, ease: "expo.inOut", stagger: 0.05 });
        });
    });
    ...
    if (marqueeContainer) {
        window.addEventListener('preloaderComplete', () => { isPreloaderComplete = true; });
        ...
        window.addEventListener("mousemove", (e) => {
            ...
            const moveToAlignLastSpan = (window.innerWidth - lastSpanRect.right) + currentContainerX;
            targetMarqueeX = moveToAlignFirstSpan + (e.clientX / window.innerWidth) * (moveToAlignLastSpan - moveToAlignFirstSpan);
        });
    }
    ...
    continueButton.addEventListener("click", () => {
        ...
        tl.to([warn, warnFooter, centerMarquee], { clipPath: "inset(0 0 100% 0)", duration: 0.5, ease: "power4.out" });
        gsap.to("nav", 1, { opacity: 1, pointerEvents: 'all', ease: "power4.out" });
    });

    // Preloader
    gsap.from(".clip-top, .clip-bottom", 2, { delay: 1, height: "50svh", ease: "power4.inOut" });
    gsap.to(".marquee", 3.5, { delay: 0.75, top: "50%", ease: "power4.inOut" });
    gsap.from(".clip-top .marquee, .clip-bottom .marquee", 5, { delay: 1, left: "100%", ease: "power4.inOut" });
    gsap.from(".clip-center .marquee", 5, { delay: 1, left: "-50%", ease: "power4.inOut" });
    gsap.to(".clip-top", 2, { delay: 6, clipPath: "inset(0 0 100% 0)", ease: "power4.inOut" });
    gsap.to(".clip-bottom", 2, {
        delay: 6,
        clipPath: "inset(100% 0 0 0)",
        ease: "power4.inOut",
        onComplete: () => window.dispatchEvent(new CustomEvent('preloaderComplete')),
    });

    if (window.innerWidth <= 979) {
        gsap.to(".clip-center .marquee span", 1, { delay: 6, opacity: 0, ease: "power2.inOut" });
    }
    ...
    if (checkbox) {
        ...
        if (savedShaderState !== null) {
            ...
            if (checkbox.checked && meetsRequirements) setTimeout(() => initializeShaders(), 100);
    ...
    function checkNavPosition() {
        if (!nav) return;
        if (menuOverride) { nav.classList.remove('on-accent'); return; }
        const navCenter = nav.getBoundingClientRect().top + nav.getBoundingClientRect().height / 2;
        let isOverAccent = false;
        document.querySelectorAll('[id="accent-bg"]').forEach(element => {
            const rect = element.getBoundingClientRect();
            if (navCenter >= rect.top && navCenter <= rect.bottom) isOverAccent = true;
        });
        nav.classList.toggle('on-accent', isOverAccent);
    }

    // Pavilion
    document.querySelectorAll(".pavilion-container").forEach((pavilionContainer, index) => {
